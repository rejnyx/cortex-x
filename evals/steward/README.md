# Steward evals (Sprint 1.6.22)

Golden eval cases for the Steward runtime — validate that the OpenRouter
...
- Cron trigger semantics (covered by `tests/integration/steward-dryrun`)
- Lock semantics (covered by `tests/unit/steward/lock.test.cjs`)

## How to run

```bash
npm test                         # eval cases run as part of full suite
node tools/run-steward-evals.cjs  # CLI runner with report output
```
...
The loader at `tests/integration/steward-evals.test.cjs` discovers cases
