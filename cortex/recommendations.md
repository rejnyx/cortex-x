...
- [ ] Investigate recurring steward nightly workflow failures [src: https://github.com/Rejnyx/cortex-x/actions/runs/27861443331]
...
### 3. [HUMAN-ONLY] Preserve input EOL style in bin/cortex-claude-md-augment.cjs
Open `bin/cortex-claude-md-augment.cjs` near lines 165-167 where the augment block injects `\n` separators. Add a helper `detectEol(content)` that returns `'\r\n'` if `content.match(/\r\n/g).length > content.match(/(?<!\r)\n/g).length / 2`, else `'\n'`. Sniff once on read, use the detected EOL throughout write operations. Today the implementation unconditionally writes `\n` which produces mixed-EOL output on Windows CRLF files and noisy git diffs. Add a fixture test `tests/unit/cortex-claude-md-augment.test.cjs` with a CRLF-input CLAUDE.md, run `--apply`, assert resulting file has zero lone-LF lines. Net change: ~20 lines of CJS + ~25 lines of test. **Marked HUMAN-ONLY 2026-05-25 after the manual nightly dispatch (run `26416312545`) confirmed deepseek-v4-flash now reaches edit-apply (engine SHA back-fill landed `c6dfa60` + injection cap raised `d08086d`) but cannot produce an implementation that passes `npm test` — verifier gate rolls back atomically every attempt. The CRLF/EOL semantics + new fixture test require operator judgment; trying it 11 days in a row burned the queue. Operator implements directly; remove the tag when done.**
