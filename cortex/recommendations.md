...
**Unmarked 2026-05-10 after Sprint 2.2.5 v0 shipped the `edit_ops[]` primitive with `kind: 'append'` that bypasses the full-file-rewrite pattern entirely. The original Day-0 dogfood block (run 25584616397) was caused by Steward emitting full-file content; Sprint 2.2.5 lets Steward emit only the appended chunk, which the engine writes via `fs.appendFileSync`. Spec-verifier `edit_position_append_grows` enforces post > pre size. Re-attempt this recommendation as the v0 dogfood test.**
...
**Unmarked 2026-05-10 after Sprint 2.2.5 v0 shipped the `kind: 'append'` primitive — exactly the case this task hit on Round 5 dogfood (run 25626029120). Steward should now emit `{"edits": [{"path": "docs/steward-usage.md", "ops": [{"kind": "append", "text": "<section>\n"}]}]}` and the engine appends byte-for-byte without touching prior content.**
