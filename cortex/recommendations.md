...
**Unmarked 2026-05-10 after Sprint 2.2.5 v1 shipped the `kind: 'str_replace'` primitive — exactly the case this task hit on Round 6 dogfood (run 25626146805). Steward should emit `{"edits": [{"path": "bin/discord-bridge/auth.cjs", "expectedSha256": "<sha>", "ops": [{"kind": "str_replace", "old_str": "function loadAllowedUserIds(", "new_str": "/**\n * @description Parse STEWARD_DISCORD_ALLOWED_USER_IDS env var into a Set of snowflake IDs.\n */\nfunction loadAllowedUserIds("}]}]}`. The anchor touches the line boundary (line-start), engine handles the splice, no rewrite of unrelated code.**
...
**Unmarked 2026-05-10 after Sprint 2.2.5 v1 shipped the `kind: 'str_replace'` primitive. This task needs TWO `str_replace` ops in the same edit: (a) anchor on `const TOOLS = Object.freeze([...])` declaration close to add the new constant after it, (b) anchor on the line just before `NAME_REGEX,` in module.exports to insert the new export. Both anchors touch line boundaries; v1 atomicity guarantees both apply or both rollback.**
