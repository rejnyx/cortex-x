...
**Marked [HUMAN-ONLY] 2026-05-10 after Round 5 autoresearch dogfood (run 25626029120) — Steward+V4Flash on a 14 KB existing file consistently returns full-file-replacement edits instead of append-only chunks (3/3 candidates triggered `no_destructive_rewrite`, file would shrink 14 KB → ~400 B). LLM lacks an append primitive; defense WORKED, no commit pushed, cost ~$0.001. Operator does the 200-500 byte append manually OR Sprint 2.3+ ships a tool-level append/insert primitive that bypasses full-content rewrite.**
