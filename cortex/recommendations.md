...
**Re-marked [HUMAN-ONLY] 2026-05-10 after Round 11 dogfood (run 25627821093) revealed an architectural gap: Sprint 2.2.5 v1 ops (str_replace + insert) require the LLM to know the file's `expectedSha256`, but cortex-x's prompt builder does NOT inject file content into the user message — only the recommendation text. The LLM therefore hallucinates the SHA (real run output: `expectedSha256=ff5a9eef58ac... but actual=f0c0446765e4...`). Engine SHA gate caught it correctly — no bad commit landed. **Defense-by-design works as designed.** Unblocking this requires Sprint 2.2.5 v1.5 (prompt-content injection: detect referenced files in action body, read + SHA + include in user prompt as `<file path="..." sha256="...">CONTENT</file>` blocks). Until v1.5 ships, str_replace + insert ops are code-correct but cannot be exercised autonomously. v0 ops (append + create) work without v1.5.**
...
**Re-marked [HUMAN-ONLY] 2026-05-10 — same architectural gap as #6 (Sprint 2.2.5 v1 ops require file content in prompt; prompt builder doesn't inject it; LLM hallucinates SHA; engine correctly rejects). Awaits Sprint 2.2.5 v1.5 prompt upgrade.**
