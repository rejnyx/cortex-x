# Hermes setup — guided activation flow

> **What this prompt does:** walks the user from "Hermes installed, never run" to "Hermes ships nightly draft PRs autonomously." Detects what's missing, fills the gaps, ends with a dry-run smoke test.
>
> **When to invoke:** user said *"set up Hermes"*, *"activate the autopilot"*, *"wire the cron"*, or pasted this prompt. Also linked from `prompts/new-project.md` Phase 6 + the `/cortex-init` skill.
>
> **Audience:** the user is technical (uses Claude Code) but may not have read `docs/hermes-usage.md`. Don't dump docs at them — execute the flow conversationally.

## What Hermes is — 30-second pitch

Open with this exact framing (adapt to detected user.yaml.language):

> **Hermes is your AI nightly autopilot.** While you sleep, it reads `cortex/recommendations.md`, picks the next action, asks the LLM (~$0.0008 per run via OpenRouter), applies the edits, runs `npm test` as a gate, opens a **draft PR**. You wake up, review the diff, merge or reject.
>
...
Wait for "OK" / "let's do it" / equivalent before proceeding. If the user has questions, answer from `docs/hermes-usage.md` § 4-level autonomy ladder.
...
□ workflow file present:      test -f .github/workflows/hermes.yml && grep -E "^name:" .github/workflows/hermes.yml
□ OPENROUTER_API_KEY in gh:   gh secret list | grep OPENROUTER_API_KEY
□ cortex-hermes on PATH:      command -v cortex-hermes
...
If file missing: scaffold a minimal one. Read `tests/fixtures/hermes-dryrun/cortex/recommendations.md` (in the cortex-x repo) for the canonical shape. Adapt slug + content to user's project.
...
```markdown
---
slug: <project-slug>      # must match repo basename — Hermes uses for journal isolation
phase: 5-adapt
date: <YYYY-MM-DD>
---
...
Each unchecked `- [ ]` item is one Hermes nightly run.
...
### 2.3 — `.github/workflows/hermes.yml` missing

Copy from cortex-x repo: `.github/workflows/hermes.yml` (in this repo as the canonical example).
...
```bash
cortex-hermes version        # → "cortex-hermes 0.1.0-pre"
```

If still missing, fall back to `node ~/cortex-x/bin/cortex-hermes.cjs <subcmd>`.
...
```bash
cortex-hermes dry-run --slug=$(basename $(git rev-parse --show-toplevel)) --json | jq .
```
...
```
✓ Hermes is wired. The next 04:00 UTC cron will:
...
Offer to halt:
- Full halt: `touch ~/.cortex/HERMES_HALT` (Hermes exits clean on next run, no spend)
- Per-repo halt: comment out the `cron:` line in `.github/workflows/hermes.yml`
- Permanent disable: delete `.github/workflows/hermes.yml`

## Phase 5 — On-demand observation

Tell user how to check Hermes status post-activation:

```bash
cortex-hermes status --slug=<your-repo>          # halt + lock + recommendations + journal rollup
gh pr list --author "Hermes (cortex-x)"          # draft PRs Hermes opened
...
- Rollup: cortex-hermes status --slug=<your-repo>
- Journal: tail ~/.cortex/journal/<your-repo>.jsonl
- Doc: docs/hermes-usage.md
```
...
- **Never modify `.github/workflows/hermes.yml` if it already exists.** Show the diff vs. canonical, ask before overwriting.
- **Never run `cortex-hermes execute` from this prompt.** That's the cron's job. Dry-run only.
