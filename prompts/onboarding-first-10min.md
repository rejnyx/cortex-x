...
- Three opt-in consent prompts (each interactive Y/n, skipped silently in CI):
  ...
  - **Discipline block append** (`cortex-claude-md-augment`) — Sprint 2.21+2.21.1, block v4 (2026-05-26 Task-tools migration). Appends R1+R2+task-list+voice+surgical-changes block to `~/.claude/CLAUDE.md` between BEGIN/END markers. `CORTEX_AUGMENT_CLAUDE_MD=0|1` env.
