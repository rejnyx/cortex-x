...
**v0.3-pre, public-ready** (Apache 2.0 relicense 2026-05-12). Tier 0 (Foundation) ✅ shipped. Tier 1 (Verification + multi-agent) ✅ shipped (Sprint 1.9 + 2.0–2.1 + 2.2.5 + 2.3a + 2.5b + 2.6b + 2.11 + 2.18 + 2.2 foundation + 2.3 mutation baseline all landed). **2026-05-14 autonomous session shipped 11 sprints + 4 R2 hardening rounds**: 2.29 (MCP recs) · 2.26 (template + steward-vs-routines + ecosystem card) · 2.20 (status flip) · 2.24 (/cortex-goal wrapper) · 2.8.2 (status flip) · 2.8.3 (agent-first audit snapshot) · 2.22 + 2.22.1 (cortex-skill-validate CLI + 7-HIGH R2) · 2.25 + 2.25.1 (cortex-dream + cortex-insights + 7-HIGH R2) · 2.3 (Stryker measure-only baseline + standards/mutation-testing.md + GHA workflow) · 2.2 + 2.2.1 (worktree-supervisor foundation: topology.cjs + standards/multi-agent-supervisor.md + recommendation_harvest_parallel action_kind + 7-HIGH R2; spawner deferred to Sprint 2.2.1) · 3.X verified-deferral (3 of 4 blockers active including NEW OpenRouter beta-header). **Sprint 2.28 → 2.28.3 chain shipped 2026-05-14** — safety-floor permissions: cortex-permissions-register CLI + install opt-in + cortex-doctor checks + 17 ship-blockers closed across 3 R2 rounds + SSOT helpers. **2955 tests, 0 failing, 2 pre-existing skips.** 18 action_kinds in Steward registry (added recommendation_harvest_parallel — registry-only, executor deferred). 6 acceptance-criterion kinds in spec-verifier (mutation_score kind reserved for Sprint 2.3.1). 14 user-facing CLI shims in `~/.claude/shared/bin/` (added cortex-skill-validate · cortex-dream · cortex-insights). 8 user-discoverable slash skills (added cortex-goal). Tier 2 essentially closed; Sprint 2.2.1 (multi-agent spawner) + 2.3.1 (mutation score criterion kind + ratchet) + 3.X (Anthropic Memory Tool unblock) are next.
