...
This pair of sprints shipped 4 commits (0deb71c → 182c310) adding +18 tests to bring the suite from 772 to 790 total. Root causes addressed include a workspace path collision where Steward targeted `.cortex-data/` files erroneously, a GH secret `\n` trap that caused malformed `apiKey` strings, confusion between provisioning and inference in the auth flow, and the LLM destructive-rewrite pattern where Steward would fabricate and replace entire file content instead of appending. Defenses shipped: a halt-check filter that blocks edits targeting `.cortex-data/` paths, apiKey trim plus KEY_MALFORMED rejection, a distinct AUTH_REJECTED error code for inference-vs-provisioning separation, the EDIT_DESTRUCTIVE_REWRITE spec-verifier with a 50% shrink threshold and mandatory `replace_all` opt-out for large rewrites, and normalized lesson hint formatting. Downstream consumers should ensure their auth flows distinguish provisioning and inference tokens, trim secrets before validation, and prefer Kind/append over full-file rewrites.
