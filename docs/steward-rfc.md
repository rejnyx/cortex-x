...
Steward powers up cortex-x to a **runtime** framework: an autonomous loop
...
## What Steward is NOT (must list)

Hardcoded Steward refusals — non-negotiable:

- **No force-push to main.** Steward can push branches, open PRs, comment
  on PRs. Never `git push --force` to a protected branch.
- **No merge without human review.** Steward opens PRs; humans merge.
- **No data deletion.** Steward never `rm -rf`, `DROP TABLE`, `truncate`,
  `supabase db reset`. block-destructive.cjs hook is the runtime
  enforcement layer (Tier 4 verified).
- **No prod restart / migration.** Steward proposes; humans execute.
...
1. **Steward core loop** — reads recommendations, picks next action,
...
4. **Rollback contract** — every Steward action = atomic git commit on a
   `hermes/<date>` branch. Revert path is `git revert HEAD`.
5. **Safety layer** — calls into block-destructive.cjs + custom Steward
   policy file (this RFC § What Steward is NOT becomes the policy).
...
Both must be green before any Steward runtime code merges:
...
- [x] Steward-policy.md drafted in `standards/` (2026-05-07) — see
...
- [x] First Steward-driven dry-run against fixture project
      ...
      suite landed 2026-05-07 (this commit). Real Steward-driven PR (with
...
3. ~~Draft `standards/steward-policy.md`.~~ ✅
4. ~~Sketch the core loop in `docs/steward-runtime.md`.~~ ✅
5. ~~Build a fixture project + first dry-run iteration in `tests/fixtures/steward-dryrun/`.~~ ✅
6. ~~Wire 6 primitives + dry-run orchestrator (no Claude SDK yet).~~ ✅
7. ~~Wire 6 primitives + dry-run orchestrator (no Claude SDK yet).~~ ✅
8. ~~Add unified `bin/cortex-steward` dispatcher CLI.~~ ✅
9. **v0.5 milestone:** integrate an LLM provider so the dry-run plan becomes
   an actual Steward commit + PR. The dry-run already produces a valid
   ...
   - **v0.5b (next):** real LLM provider. **Updated 2026-05-07:** the
     ...
     zero-deps). See `docs/steward-runtime.md` § 4.5 for the full design.
