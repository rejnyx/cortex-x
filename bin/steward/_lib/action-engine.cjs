...
const STEWARD_SYSTEM_PROMPT = [
  ...
  '',
  'Sprint 2.2.5 v0+v1 ops (4 kinds shipped; delete_file is Sprint 2.2.6 deferred):',
  '',
  ...
  '',
  '  {"kind": "str_replace", "old_str": "<unique anchor>", "new_str": "<replacement>"}',
  '    — Use when the action says "modify line X", "fix this typo", "rename foo to bar in file Y".',
  '    — `old_str` MUST occur exactly once in the file (uniqueness enforced; ambiguous → EDIT_OP_ANCHOR_AMBIGUOUS).',
  '    — `old_str` MUST touch a line boundary (start at column 0 OR end at \\n). For files in',
  '      bin/steward/, bin/cortex/, standards/, .github/, profiles/, prompts/ this is HARD-blocked',
  ...
  '  • LLM-as-code defense: when target is *.js / *.cjs / *.mjs / *.ts / *.tsx, content',
  '    matching shell-eval / process-injection patterns (require("child_process"), eval(),',
  '    new Function(), ${process.env.X}, spawn/exec calls) is blocked unconditionally',
  '    via EDIT_OP_LLM_CONTENT_DANGEROUS. Use the legacy `replace_all` path with explicit',
  '    operator authorization if you genuinely need to emit such code.',
  '',
  'PREFER ops shape order: append > create > insert > str_replace > legacy {content}.',
  'When in doubt, the most-specific op wins (insert beats append for "add JSDoc above").',
  '',
  'Rules:',
