// policy-check.cjs — Steward Ring 1 denylist (over block-destructive Ring 2).
//
// Patterns from standards/steward-policy.md § 3 Denylist. Runs BEFORE any tool
...
//   - Patterns mirror standards/steward-policy.md authoritative list
...
const STEWARD_DENY = [
  ...
  // Auto-merge prevention (Hermes opens PRs, humans merge)
  {
    ...
    reason: 'Steward cannot merge PRs — humans merge',
  },
  {
    code: 'NO_INTEGRATION_MERGE',
    p: /\bgit\s+merge\s+(main|master)\b/,
    reason: 'Steward cannot merge to integration branch',
  },
  ...
  // Force-push (already in block-destructive but Ring 1 catches first)
  {
    ...
    reason: 'Steward cannot force-push',
  },
  ...
  // Hard reset on pushed branches
  {
    ...
    reason: 'Steward cannot hard-reset (use git revert)',
  },
  ...
  // exfiltrate live keys.
  {
    ...
    reason: 'Steward cannot read secret material (.env*, *.pem, *.key, secrets/, .ssh/, .gnupg/)',
  },
  {
    code: 'NO_SECRET_PIPE',
    p: /\S*?(?:\.env(?:\.|\b)|\.pem\b|\.key\b|secrets?\/|\.ssh\/|\.gnupg\/)\S*\s*\|/i,
    reason: 'Steward cannot pipe secret material to other commands',
  },
];
...
// can't reliably detect "Steward is editing a human_only path" because args are
