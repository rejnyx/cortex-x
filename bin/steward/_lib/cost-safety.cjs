...
//
// Defaults (env-overridable; HERMES_* aliases honored through Sprint 4.7
// rebrand backward-compat layer in env.cjs, removed in v0.2.0):
//   STEWARD_DAILY_USD_CAP       = 5    (existing in execute.cjs)
//   STEWARD_WEEKLY_USD_CAP      = 25   (Sprint 1.9.1)
//   STEWARD_MONTHLY_USD_CAP     = 80   (Sprint 1.9.1)
//   STEWARD_TOKEN_VELOCITY_CAP  = 50000 tokens / 5min (Sprint 1.9.1)
//   STEWARD_LOOP_THRESHOLD      = 5    (Sprint 1.9.1 — same criterion 5x in window)
//   STEWARD_LOOP_WINDOW_DAYS    = 7    (Sprint 1.9.1)
//
...
// `name` is the suffix after STEWARD_/HERMES_; readEnv() handles the prefix
...
function readWeeklyCap() { return readEnvFloat('WEEKLY_USD_CAP', DEFAULT_WEEKLY_USD_CAP); }
function readMonthlyCap() { return readEnvFloat('MONTHLY_USD_CAP', DEFAULT_MONTHLY_USD_CAP); }
function readTokenVelocityCap() { return readEnvInt('TOKEN_VELOCITY_CAP', DEFAULT_TOKEN_VELOCITY_CAP); }
function readLoopThreshold() { return readEnvInt('LOOP_THRESHOLD', DEFAULT_LOOP_THRESHOLD); }
function readLoopWindowDays() { return readEnvInt('LOOP_WINDOW_DAYS', DEFAULT_LOOP_WINDOW_DAYS); }
...
function spendForecast(slug) {
  ...
  // Daily
  const dailyCap = readEnvFloat('DAILY_USD_CAP', DEFAULT_DAILY_USD_CAP);
