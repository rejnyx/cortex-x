...
| `cortex-usage` | _(direct)_ | `cortex-usage.cjs` | Read-only usage rollup over the post-tool-use journal — classifies `Read` events into cortex artifacts (standards/prompts/agents/skills) and reports hot (earns its context cost) vs cold prune candidates. Root-independent keys (repo + installed copy fold to one). Fail-open. Flags: `--since`, `--kind`, `--cold`, `--json`. Sprint 2.38. |
