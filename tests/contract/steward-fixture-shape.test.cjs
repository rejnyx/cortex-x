...
/**
 * Contract test — tests/fixtures/steward-dryrun/ shape
 *
 * The steward-dryrun fixture is the deterministic dry-run target Hermes (Phase 7)
 ...
 *
 * Companion docs: standards/steward-policy.md, docs/steward-runtime.md.
 */
...
const FIXTURE_ROOT = path.join(REPO_ROOT, 'tests', 'fixtures', 'steward-dryrun');
...
describe('steward-dryrun fixture: structural shape', () => {
  test('fixture root exists', () => {
    assert.ok(fixtureExists('.'), 'tests/fixtures/steward-dryrun/ must exist');
  });
  ...
  test('README.md present and explains fixture purpose', () => {
    ...
    assert.match(readme, /Steward/, 'README must reference Steward runtime context');
  });
...
describe('steward-dryrun fixture: recommendations.md parseable contract', () => {
  ...
  test('frontmatter contains slug field', () => {
    ...
    assert.match(fm[1], /^slug:\s*steward-dryrun\b/m, 'frontmatter must declare slug: steward-dryrun');
  });
...
describe('steward-dryrun fixture: PII + env safety', () => {
  const filesToScan = [
    'README.md',
    'CLAUDE.md',
    'package.json',
...
describe('steward-dryrun fixture: package.json hygiene', () => {
...
describe('steward-dryrun fixture: smoke test runs cleanly', () => {
