...
const cli = require('../../../bin/cortex-steward.cjs');

const FIXTURE_SRC = path.resolve(__dirname, '..', '..', 'fixtures', 'steward-dryrun');
const SLUG = 'steward-dryrun';
const CLI_PATH = path.resolve(__dirname, '..', '..', '..', 'bin', 'cortex-steward.cjs');
...
describe('cortex-steward: help + version', () => {
  ...
  test('--version prints version line', () => {
    ...
    assert.equal(result.status, 0);
    assert.match(result.stdout, /^cortex-steward \S+/);
  });
...
describe('cortex-steward: subcommand dispatch', () => {
  test('dry-run dispatches to bin/steward/dry-run.cjs', () => {
  ...
  test('status dispatches to bin/steward/status.cjs', () => {
...
describe('cortex-steward: error paths', () => {
  test('unknown subcommand exits 1 with hint', () => {
    ...
    assert.match(result.stderr, /cortex-steward help/);
  });
