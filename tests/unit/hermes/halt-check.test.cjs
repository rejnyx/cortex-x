...
const {
  ...
  EX_TEMPFAIL,
  SENTINEL_FILENAME,
} = require('../../../bin/hermes/_lib/halt-check.cjs');

function tmpDir(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `hermes-halt-${prefix}-`));
}

function tmpDataHome(prefix) {
  const dir = tmpDir(prefix);
  // Pre-create the .cortex root so resolveCortexDataHome doesn't pick a different default
  return dir;
}
...
describe('halt-check: project sentinel', () => {
  test('per-repo HERMES_HALT triggers halt', () => {
...
describe('halt-check: fleet sentinel', () => {
  test('CORTEX_DATA_HOME/HERMES_HALT triggers halt', () => {
...
describe('halt-check: contract', () => {
  ...
  test('SENTINEL_FILENAME is HERMES_HALT (matches policy doc)', () => {
    assert.equal(SENTINEL_FILENAME, 'HERMES_HALT');
  });
