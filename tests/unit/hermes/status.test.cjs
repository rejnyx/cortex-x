...
const status = require('../../../bin/hermes/status.cjs');
...
function freshFixture(prefix) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `hermes-status-${prefix}-`));
...
describe('status: clean fixture', () => {
  test('reports not-halted, lock free, recommendations OK', () => {
    ...
    const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-status-data-'));
...
describe('status: after dry-run activity', () => {
  ...
  test('cost + token totals roll up correctly', () => {
    const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-status-cost-'));
    withDataHome(dataHome, () => {
      ...
      const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-status-cost-repo-'));
...
describe('status: human-readable formatting', () => {
  test('formatHumanReadable produces multi-line text', () => {
    ...
    withDataHome(dataHome, () => {
      ...
      assert.match(text, /Hermes status/);
...
describe('status: CLI', () => {
  test('CLI --json emits structured status', () => {
    ...
    const result = spawnSync(process.execPath, [
      path.resolve(__dirname, '..', '..', '..', 'bin', 'hermes', 'status.cjs'),
      `--slug=${SLUG}`,
      `--repo-root=${repoRoot}`,
      '--json',
    ], {
