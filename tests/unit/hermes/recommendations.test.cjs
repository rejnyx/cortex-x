...
const {
  ...
  extractCitations,
  pickNextAction,
} = require('../../../bin/hermes/_lib/recommendations.cjs');
...
describe('recommendations: integration with hermes-dryrun fixture', () => {
  test('parses the real hermes-dryrun fixture without error', () => {
    const fixturePath = path.resolve(
      __dirname, '..', '..', 'fixtures', 'hermes-dryrun', 'cortex', 'recommendations.md'
    );
    const parsed = parseRecommendations(fixturePath);
    assert.equal(parsed.frontmatter.slug, 'hermes-dryrun');
    assert.ok(parsed.sections['DO this week']);
