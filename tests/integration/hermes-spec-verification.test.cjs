...
describe('spec-verifier integration: PR #3 reproduction (destructive rewrite of large doc)', () => {
  test('1000-byte file shrunk to 4 bytes → SPEC_VIOLATION + rollback', async () => {
    const repoRoot = tmpRepoWith({ 'docs/hermes-usage.md': 'x'.repeat(1000) });
    ...
    await withEnv({
      ...
      HERMES_MOCK_PLAN: JSON.stringify({
        edits: [{ path: 'docs/hermes-usage.md', content: 'tiny' }],
      }),
    }, async () => {
      ...
      const restored = fs.readFileSync(path.join(repoRoot, 'docs/hermes-usage.md'), 'utf8');
