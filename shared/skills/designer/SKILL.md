---
name: designer
description: Senior front-end designer flow — Phase 0 DS bootstrap from repo (Claude Design), Phase 1 intake with image/sketch/vibe inputs (Stitch Vibe), Phase 1.5 plan gate (Lovable), Phase 2 library palette + image-gen MCP, Phase 3 parallel worktree exploration with DS-conformance gate (Magic Patterns), Phase 4 live dev server iteration, Phase 5 handoff via Stitch DESIGN.md + STYLE.md SSOT split. Opt-in Award mode (`--award` flag) overrides phases with Awwwards-SOTD discipline derived from reverse-engineering 25 SOTD 2025-2026 winners. Replicates "Claude Design" inside Claude Code without the weekly-limit cliff. Auto-discovered from ~/.claude/skills/designer/SKILL.md after install. Invoke as "/designer" or "navrhni mi web"; add `--award` for portfolio/agency/SOTD-targeted work.
